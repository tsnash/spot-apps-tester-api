...
public class RelationshipPreferenceRepositoryTest extends AbstractRepositoryTestBase {

    @Autowired
    private RelationshipPreferenceRepository repository;
    ...
    void mapsIdIsDerivedFromUserProfile() {
        ...
        RelationshipPreference saved = repository.save(pref);
        assertThat(saved.getUserId()).isNotNull().isEqualTo(userProfile.getUserId());
        entityManager.flush();
        entityManager.clear();

        RelationshipPreference retrieved = repository.findById(saved.getUserId()).orElseThrow();
