...
public class PetsPreferenceRepositoryTest extends AbstractRepositoryTestBase {

    @Autowired
    private PetsPreferenceRepository repository;
    ...
    void mapsIdIsDerivedFromUserProfile() {
        ...
        PetsPreference pref = new PetsPreference();
        pref.setUserProfile(userProfile);

        PetsPreference saved = repository.save(pref);
        assertThat(saved.getUserId()).isNotNull().isEqualTo(userProfile.getUserId());
        entityManager.flush();
        entityManager.clear();

        PetsPreference retrieved = repository.findById(saved.getUserId()).orElseThrow();
