...
public class EducationPreferenceRepositoryTest extends AbstractRepositoryTestBase {

    @Autowired
    private EducationPreferenceRepository repository;
    ...
    void mapsIdIsDerivedFromUserProfile() {
        ...
        EducationPreference saved = repository.save(pref);
        assertThat(saved.getUserId()).isNotNull().isEqualTo(userProfile.getUserId());
        entityManager.flush();
        entityManager.clear();

        EducationPreference retrieved = repository.findById(saved.getUserId()).orElseThrow();
