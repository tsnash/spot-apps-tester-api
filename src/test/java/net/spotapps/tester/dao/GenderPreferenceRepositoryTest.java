...
public class GenderPreferenceRepositoryTest extends AbstractRepositoryTestBase {

    @Autowired
    private GenderPreferenceRepository repository;
    ...
    void mapsIdIsDerivedFromUserProfile() {
        ...
        GenderPreference pref = new GenderPreference();
        pref.setUserProfile(userProfile);

        GenderPreference saved = repository.save(pref);
        assertThat(saved.getUserId()).isNotNull().isEqualTo(userProfile.getUserId());
        entityManager.flush();
        entityManager.clear();

        GenderPreference retrieved = repository.findById(saved.getUserId()).orElseThrow();
